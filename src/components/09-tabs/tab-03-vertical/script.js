// * ----------------------------------------------------------------------------
// * COMPONENTE: Pestañas 03 - Verticales (Vertical Tabs)
// * AUTONOMÍA ESTRICTA: Reutiliza la lógica funcional del componente Base.
// * Se modificó el ID del contenedor (tabComponentVertical) para asegurar la autonomía.
// * ----------------------------------------------------------------------------

document.addEventListener('DOMContentLoaded', () => {
    // 1. Obtener el contenedor autónomo del componente de pestañas (ID específico del snippet)
    const tabContainer = document.getElementById('tabComponentVertical');
    ...
    // NOTA: Se mantienen las flechas Izquierda/Derecha por convención horizontal,
    // pero idealmente en vertical se usarían Arriba/Abajo. Mantenemos la lógica de la base.
    tabList.addEventListener('keydown', (event) => {
        let currentActiveTab = tabList.querySelector('.tab-item.is-active');
        let nextTab = null;
        
        switch (event.key) {
            case 'ArrowDown': // Usar Down/Up en vertical es más intuitivo
            case 'ArrowRight':
